...
const loginUser = async (req, res , next) => {
  try {
    ...
    let token = jwt.sign({userId: userDetails._id}, process.env.SECRECT_KEY, { expiresIn:'60h' })
    ...
    return res.json({ message: "User login sucessfull", name: userDetails?.name , token });
